...
bool EqivaKeyBle::sendMessage(eQ3Message::Message *msg, bool nonce) {
    if (((sendingNonce == false && clientState.remote_session_nonce.length() > 0) || nonce) && this->state() == espbt::ClientState::ESTABLISHED) {
      ...
      ESP_LOGI(TAG, "Waiting for connection... (sendingNonce: %d) (remoteSessionNonce: %d) (remoteSessionNonce: %d) (clientStateEstablished: %d)", sendingNonce, clientState.remote_session_nonce.length() > 0, this->state() == espbt::ClientState::ESTABLISHED);
      currentMsg = msg;
      return false;
    }
}
